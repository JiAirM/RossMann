...
# This path should match the subfolder in your GitHub repository
...
try:
    ...
    st.error(f"Error: Model file not found at {model_path}. Please ensure the model file is in the '{DATA_DIR}' directory within your repository.")
    st.stop()
except EOFError:
    st.error(f"Error loading model file at {model_path}. The file might be incomplete or corrupted. Ensure it was uploaded correctly with Git LFS if it's large.")
    st.stop()
...
try:
    ...
    st.error(f"Error: {train_store_transformed_df_path} not found. Please ensure the file is in the '{DATA_DIR}' directory within your repository.")
    st.stop()
except Exception as e:
    st.error(f"An unexpected error occurred while loading {train_store_transformed_df_path}: {e}")
    st.stop()
...
def engineer_promotion_related_features(df):
    ...
    For simplicity in this example, we'll apply it to the single row input.
    """
    ...
    # This cumulative sum logic is for a time series. For a single row, this will likely be 0 or incorrect.
    # A real deployment needs a lookup table or precomputed values.
    # Placeholder for demonstration:
    df['DaysSinceLastPromo'] = 0 # Default for single prediction

    # Calculate 'Number of consecutive promotional days'
    # Placeholder for demonstration:
    df['ConsecutivePromoDays'] = 0 # Default for single prediction
...
def create_binned_competition_distance(df, original_competition_distance_series):
    ...
    try:
        ...
        # Ensure the column exists and is numeric before binning
        if 'CompetitionDistance' in df.columns and pd.api.types.is_numeric_dtype(df['CompetitionDistance']):
             # Check if there are enough unique non-NaN values to create 5 bins
             if original_competition_distance_series.nunique() >= 5:
                df['CompetitionDistance_Bin'] = pd.qcut(df['CompetitionDistance'], q=5, labels=False, duplicates='drop')
             else:
                 st.warning("Not enough unique CompetitionDistance values for 5 bins. Creating a single bin.")
                 df['CompetitionDistance_Bin'] = 0 # Create a single bin or handle differently
        else:
            st.warning("CompetitionDistance column not found or not numeric for binning.")
            df['CompetitionDistance_Bin'] = -1 # Indicate binning failed


    except Exception as e:
        st.warning(f"Could not create CompetitionDistance_Bin. Error: {e}")
...
def prepare_input_features(input_df, original_train_df_for_features, original_train_store_df_for_features):
    ...
    if hasattr(refined_xgb_model, 'feature_names_in_'):
        ...
        # This list is derived from the final X_train.columns before training the refined model in the notebook.
...
input_data = {
    ...
    'CompetitionOpenSinceMonth': None, # Placeholder
    'CompetitionOpenSinceYear': None, # Placeholder
    'Promo2SinceWeek': None, # Placeholder
    'Promo2SinceYear': None, # Placeholder
}
...
if st.sidebar.button('Predict Sales'):
    ...
    train_df_path_features = os.path.join(DATA_DIR, 'train.csv')
    train_store_df_path_original_features = os.path.join(DATA_DIR, 'train_store.csv') # Use the merged data before sampling/optimization

    try:
        original_train_df_for_features = pd.read_csv(train_df_path_features)
        original_train_store_df_for_features = pd.read_csv(train_store_df_path_original_features)
    except FileNotFoundError:
        st.error(f"Error: Original training data files (train.csv or train_store.csv) not found in the '{DATA_DIR}' directory within your repository.")
        st.stop()
    except Exception as e:
        st.error(f"An unexpected error occurred while loading original training data: {e}")
        st.stop()
    ...
    if hasattr(refined_xgb_model, 'feature_names_in_'):
        model_features_order = refined_xgb_model.feature_names_in_
         ...
         # Fallback list of features from the notebook's final refined model training
         # This list MUST match the exact columns and order used to train refined_xgb_model
         model_features_order = [
            'Store', 'DayOfWeek', 'Customers', 'Open', 'Promo', 'SchoolHoliday',
            'AvgBasket', 'CompetitionDistance', 'Promo2', 'DaysSinceLastPromo',
            'ConsecutivePromoDays', 'CompetitionDistance_log', 'CompetitionOpen',
    ...
    try:
        ...
        st.write(f'The predicted sales are: {prediction[0]:,.2f}')
         ...
         st.write(processed_input.columns.tolist())
         # Display data types to help debugging
         st.write("Processed Input DataFrame dtypes:")
         st.write(processed_input.dtypes)
